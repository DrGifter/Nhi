...
let scene2Started = false;     // đảm bảo chỉ chạy scene 2 một lần
let matrixLoop = null;         // lưu ID của setInterval để clear
    ...
    if (now - explosionStartTime > EXPLOSION_TIME) {
      ...
      if (currentTextIndex >= texts.length) {
        if (!scene2Started) {
          ...
          // Ẩn canvas, hiện scene 2
          document.getElementById("second-scene").style.display = "block";
        ...
        return; // THOÁT hẳn drawMatrixRain
      }
      currentCharIndex = 0;
...
// ===== Chỉ chạy matrix sau khi bấm nút =====
function startMatrix() {
  currentDisplayTime = BASE_DISPLAY_TIME + (texts[0].length > 4 ? 2000 : 0);
  initDots(texts[currentTextIndex]);
  lastChangeTime = Date.now();
  lastCharTime = Date.now();
  matrixLoop = setInterval(drawMatrixRain, 50);
}

btn.addEventListener("click", () => {
  sound.play().then(() => {
    // Khi nhạc phát thành công thì mới bắt đầu matrix
    startMatrix();
